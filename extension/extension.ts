...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidOpenTextDocument((doc) =>
      runAnalysis(doc, diagnosticCollection)
    )
  );
  context.subscriptions.push(
    vscode.workspace.onDidChangeTextDocument((evt) =>
      runAnalysis(evt.document, diagnosticCollection)
    )
  );
    ...
    vscode.commands.registerCommand("a11y.fixAll", async () => {
      ...
      if (!editor) {
        vscode.window.showInformationMessage(
          "Open a file to apply A11y fixes."
        );
        return;
      }
      ...
      if (!["javascriptreact", "typescriptreact"].includes(doc.languageId)) {
        vscode.window.showInformationMessage(
          "A11y Fix All works only on JSX/TSX files."
        );
        return;
      }
      ...
      try {
        ...
        const fileIssues = issues.filter(
          (i) => Array.isArray(i.fix?.edits) && i.fix.edits.length > 0
        );

        if (fileIssues.length === 0) {
          vscode.window.showInformationMessage(
            "No auto-fixable accessibility issues found."
          );
          return;
        }

        // Confirmation (list up to first 5 rules)
        const sample = Array.from(new Set(fileIssues.map((i) => i.id)))
          .slice(0, 5)
          .join(", ");
        const confirm = await vscode.window.showWarningMessage(
          `Apply ${
            fileIssues.length
          } accessibility fixes in this file? Rules: ${sample}${
            fileIssues.length > 5 ? ", ..." : ""
          }`,
          { modal: true },
          "Apply"
        ...
        if (ok) {
          ...
          // await doc.save();
          vscode.window.showInformationMessage(
            `Applied ${fileIssues.length} accessibility fixes.`
          );
        ...
        vscode.window.showErrorMessage(
          "A11y Fix All failed. See console for details."
        );
      }
    })
  );
  ...
  try {
    ...
    console.log(
      "registerWebviewViewProvider called for viewType:",
      A11yPanelProvider.viewType
    );
  } catch (err) {
    ...
    vscode.commands.registerCommand("a11y.showPanel", async () => {
      try {
        ...
        await vscode.commands.executeCommand(
          "workbench.view.extension.a11yView"
        );
        try {
          await vscode.commands.executeCommand(
          ...
          console.log(
            "workbench.action.openView called for view id: a11yPanel"
          );
        } catch (errOpen) {
        ...
        try {
          if (typeof (panelProvider as any)?.refresh === "function") {
            ...
            console.log(
              "panelProvider.refresh() not available at showPanel time"
            );
          }
        } catch (errRefresh) {
...
function buildWorkspaceEditForIssues(
  ...
  try {
    ...
    const diagnostics: vscode.Diagnostic[] = issues.map((issue) => {
      ...
      const startPos = new vscode.Position(
        Math.max(0, issue.start.line),
        Math.max(0, issue.start.column)
      );
      const endPos = new vscode.Position(
        maxLine,
        Math.max(0, issue.end.column)
      );
...
class A11yQuickFixProvider implements vscode.CodeActionProvider {
    ...
    try {
      ...
      for (const diagnostic of context.diagnostics) {
        ...
        if (
          !issue ||
          !issue.fix ||
          !Array.isArray(issue.fix.edits) ||
          issue.fix.edits.length === 0
        )
          continue;

        const action = this.buildCodeActionForIssue(
          issue,
          diagnostic,
          document
        );
  ...
  private buildCodeActionForIssue(
    issue: Issue,
    ...
    try {
      ...
      for (const e of issue.fix!.edits) {
        const startLine = Math.max(
          0,
          Math.min(e.start.line, document.lineCount - 1)
        );
        const endLine = Math.max(
          0,
          Math.min(e.end.line, document.lineCount - 1)
        );
